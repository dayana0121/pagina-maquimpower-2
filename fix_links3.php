...
foreach ($iterator as $file) {
    if ($file->isFile()) {
        ...
        if (in_array($ext, ['php', 'js', 'html'])) {
            ...
            $nuevoContenido = preg_replace('#(/pagina_dos)+/#', '/', $contenido);

            // Reemplazar baseUrl si tiene múltiples /pagina
            $nuevoContenido = preg_replace('#"//pagina#_dos', '"//pagina_dos', $nuevoContenido); // Por si acaso
            $nuevoContenido = preg_replace('#\$_SERVER\[\'HTTP_HOST\'\] \. "(/pagina_dos)+"#', '$_SERVER[\'HTTP_HOST\'] . "/pagina_dos"', $nuevoContenido);
